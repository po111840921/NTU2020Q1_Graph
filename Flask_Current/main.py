...
# 					result = bar.producedata()
	...
	filename_var = session.get('filename_var', None)
	session['filename_var'] = filename_var
	return render_template("bar.html",confirm = 2,send_file = filename_var )
...
def submit_color_button_curve():
	if request.method == 'POST':
		...
		image.LockCurve()
		cv2.imwrite('/Users/Kyle/Desktop/Flask_Current/static/output1.png',image.photo)
		image.smooth()
		data = image.output()
...
def curve_scuccess():
	if request.method == 'POST':
		filename_var = session.get('filename_var', None)
	return render_template("Curve.html",confirm = 3,send_file = filename_var, download = UPLOAD_FOLDER+'/'+filename_var)
...
def spotcurve_scuccess():
	if request.method == 'POST':
		...
		image.CurveRecognize()
		image.LockCurve()
		cv2.imwrite('/Users/Kyle/Desktop/Flask_Current/static/output.png',image.photo)
		image.smooth()
		data = image.output()
